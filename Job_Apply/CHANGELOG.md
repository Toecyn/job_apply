...
## 2026-08-22 — User profile replaces hardcoded resume and search config
**Problem:** Every target title, exclusion keyword, search market, and the resume itself was either a Python constant (scraper.py, scorer.py) or a static file (master_resume.json). Reusing this system for a different job search meant editing source code.
**Root cause:** No stored application state for "who is searching and for what" — configuration and code were the same thing.
**Fix:** Added a single-row `profile` table (schema.sql) and profile_store.py as the sole read/write path for it. scraper.py, scorer.py, tailor.py, and batch_scorer.py now read target titles, exclusions, scoring keywords, search markets, and resume content from the profile instead of master_resume.json and hardcoded lists. Added a Settings page (/settings): CV upload -> one Claude call drafts the structured resume -> human reviews and confirms before it saves, mirroring the existing intake-review-generate pattern in tailor.py. Added Vercel Blob storage (storage.py) for the uploaded CV and generated tailored resumes, replacing a local output/ directory that never actually persisted on Vercel's serverless filesystem. Search markets are now an arbitrary list of location/mode/country rows instead of six fixed scrape passes, and the dashboard's market stat cards and filter now render from that same list.
**Result:** Verified end to end on the live deployment against a real reconfigured profile (different target titles, Canada/Nigeria markets) — scraping, scoring, and the dashboard all now follow whatever is saved in Settings.
**Pattern:** Externalize configuration from code. Anything one user of the system would need to change to point it at a different job search belongs in stored state, not source.

---

## 2026-08-22 — Production hardening: five silent failures found via live testing
**Problem:** Several features 500'd or silently did nothing the first time they were exercised against the live Vercel deployment, each with no visible explanation: a 404 on a retired Claude model, an AttributeError from Claude's default thinking output, truncated JSON on resume extraction, a 500 on the very first profile save, a Postgres type error on a boolean setting, and a Refresh Jobs button that appeared to do nothing at all.
**Root cause:** Six independent issues, each invisible until run for real: claude-sonnet-4-20250514 had been retired from the API; Claude Sonnet 5 runs adaptive thinking by default, which broke every `response.content[0].text` call site in the codebase (nine of them, across six files); a 4000-token cap was too low for a full resume-shaped JSON response; init_db() only ever ran as a side effect of a scrape, so the new profile table never got created on a deploy that hadn't scraped yet; Postgres rejects a Python bool against an INTEGER column outright; and python-jobspy — required to import scraper.py at all — was deliberately excluded from the Vercel-deployed requirements.txt, so Refresh Jobs could never have scraped anything, on any deploy, ever.
**Fix:** Migrated every Claude call to claude-sonnet-5. Added a shared ai_logger.extract_text() helper that scans for the first text-type content block instead of assuming position 0. Raised max_tokens to 16000 on the two calls that generate full-resume JSON. Moved init_db() into db.py and call it unconditionally at app startup instead of only after a scrape. Made profile_store.save_profile() coerce booleans to integers generically. Added python-jobspy to requirements.txt and a maxDuration to vercel.json. Replaced every bare except-and-redirect in the affected routes with real error messages that actually reach the browser.
**Result:** Each fix was confirmed against the exact error reported from the live site, one at a time, until that symptom cleared.
**Pattern:** An exception caught and only printed to a server log nobody can see is indistinguishable from success, from the outside. The fastest debugging tool available here wasn't guessing at causes — it was making every user-facing route say what actually happened.
